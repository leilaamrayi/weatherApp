<template>
  <div class="search-input">
    <input ref="searchInput" type="text" v-model="location" @blur="search" @input="resetTimer"
...
export default defineComponent({
  ...
  mounted() {
    ...
    if (locationParam) {
      ...
      this.search(); // Start searching immediately after getting user location
    }
  },
  methods: {
    search() {
    ...
    getCityName(latitude: number, longitude: number) {
      const apiUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=da2c5748f467c455a89d0b1a2e32a0b2&units=metric`;
